  ...
  return (
    <Shell role={role} name={profile.full_name ?? ""}>
      ...
      <form className="mb-3 flex gap-2">
        <input name="trigger" defaultValue={searchParams.trigger} placeholder="Filter trigger…" className="rounded-lg border border-stone-300 p-2 text-sm dark:border-stone-700 dark:bg-stone-800" />
        <select name="status" defaultValue={searchParams.status} className="rounded-lg border border-stone-300 p-2 text-sm dark:border-stone-700 dark:bg-stone-800">
          <option value="">any status</option><option>ok</option><option>error</option><option>skipped</option>
        </select>
        <button className="rounded-lg bg-moss px-3 py-2 text-sm text-white">Filter</button>
      </form>
      <div className="mb-8 overflow-x-auto rounded-xl bg-white shadow-sm dark:bg-stone-900">
        <table className="w-full text-xs">
          <thead><tr className="border-b border-stone-200 text-left text-stone-500 dark:border-stone-700"><th className="p-2">When</th><th className="p-2">Trigger</th><th className="p-2">Ref</th><th className="p-2">Status</th><th className="p-2">Detail</th></tr></thead>
          <tbody>
            {(runs ?? []).map((r) => (
              <tr key={r.id} className="border-b border-stone-100 dark:border-stone-800">
                <td className="p-2 text-stone-400">{new Date(r.created_at).toLocaleString("en-US", { timeZone: "America/Denver" })}</td>
                <td className="p-2 font-mono">{r.trigger}</td>
                <td className="p-2 font-mono">{r.ref_id?.slice(0, 8) ?? ""}</td>
                <td className={`p-2 ${r.status === "error" ? "text-red-600" : r.status === "skipped" ? "text-amber-600" : "text-green-700"}`}>{r.status}</td>
      ...
      <div className="overflow-x-auto rounded-xl bg-white shadow-sm dark:bg-stone-900">
        <table className="w-full text-xs">
          <thead><tr className="border-b border-stone-200 text-left text-stone-500 dark:border-stone-700"><th className="p-2">Phone</th><th className="p-2">Body</th><th className="p-2">Send after</th><th className="p-2">State</th></tr></thead>
          <tbody>
            {(queue ?? []).map((s) => (
            ...
            {!queue?.length && <tr><td colSpan={4} className="p-4 text-stone-400">Queue empty.</td></tr>}
          </tbody>
        </table>
      </div>
    </Shell>
