          ...
          <>
            <div className="mb-2 flex items-center justify-between">
              ...
              <button onClick={toggleTakeover} className={`rounded-full px-3 py-1 text-xs ${activeThread.escalated ? "bg-red/15 text-red" : "bg-ice/15 dark:bg-white/10"}`}>
                {activeThread.escalated ? "Human has thread — hand back to Norma" : "Take over from Norma"}
              </button>
            </div>
            <div className="mb-3 max-h-[45vh] space-y-2 overflow-y-auto">
              {messages.map((m: any) => (
                <div key={m.id} className={`max-w-[76%] ${m.direction === "inbound" ? "" : "ml-auto"}`}>
                  ...
                  <div className={`mt-0.5 text-[10px] text-[color:var(--body)]/70 ${m.direction === "inbound" ? "" : "text-right"}`}>{m.sender === "nora" ? "Norma" : m.sender} · {new Date(m.created_at).toLocaleString("en-US", { timeZone: "America/Denver", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</div>
                </div>
              ))}
            </div>
