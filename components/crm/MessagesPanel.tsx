      ...
      <div className="mo-card p-2 lg:overflow-y-auto">
        {threads.map((t: any) => (
          <Link key={t.id} href={`/crm/messages?thread=${t.id}`}
            className={`flex items-center gap-2.5 rounded-xl p-2 text-sm transition ${t.id === activeThread?.id ? "bg-teal text-white" : "hover:bg-black/[0.04]"}`}>
          ...
          <>
            ...
            <div className="mb-3 max-h-[45vh] space-y-2 overflow-y-auto">
              {messages.map((m: any) => (
                <div key={m.id} className={`max-w-[76%] ${m.direction === "inbound" ? "" : "ml-auto"}`}>
                  <div className={`rounded-2xl px-3 py-2 text-sm leading-relaxed ${m.direction === "inbound" ? "rounded-bl-md bg-black/[0.05] text-[color:var(--ink)]" : "rounded-br-md bg-teal text-white"}`}>{m.body}</div>
            ...
            <div className="flex gap-2">
              <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Type a message…"
                className="h-10 flex-1 rounded-full border border-[color:var(--border)] bg-white/70 px-4 text-sm outline-none focus:border-teal dark:bg-white/10" />
