...
export function TodayStops({ jobs }: { jobs: Stop[] }) {
  ...
  return (
    <div className="space-y-3">
      {queued > 0 && <div className="rounded-lg bg-amber-50 p-2 text-sm dark:bg-amber-950">📶 {queued} update(s) queued offline — will retry automatically.</div>}
      {jobs.map((j, i) => (
        <div key={j.id} className={`rounded-xl bg-white p-4 shadow-sm dark:bg-stone-900 ${j.weather_flag ? "border border-amber-400" : ""}`}>
          <div className="mb-1 flex items-center justify-between">
            <span className="font-semibold">{i + 1}. {j.customers?.full_name ?? "Customer"}</span>
            <span className="text-xs text-stone-500">{j.status}{j.weather_flag ? " ☔" : ""}</span>
          </div>
          <p className="text-sm text-stone-600 dark:text-stone-300">{j.properties?.address}, {j.properties?.city} · {j.services?.name}</p>
          {(j.properties?.gate_code || j.properties?.pets || j.properties?.access_notes) && (
            <p className="mt-1 text-xs text-stone-500">{[j.properties?.gate_code && `gate ${j.properties.gate_code}`, j.properties?.pets && `pets: ${j.properties.pets}`, j.properties?.access_notes].filter(Boolean).join(" · ")}</p>
          )}
          <div className="mt-3 flex flex-wrap gap-2">
            {!j.arrival_at && (
              <button disabled={busy === j.id} onClick={() => fire("/api/crm/clock", { job_id: j.id, event: "arrived" }, j.id)}
                className="rounded-lg bg-moss px-4 py-2 text-sm font-semibold text-white">Clock in</button>
            )}
            {j.arrival_at && !j.departure_at && (
              <button disabled={busy === j.id} onClick={() => fire("/api/crm/clock", { job_id: j.id, event: "departed" }, j.id)}
                className="rounded-lg bg-clay px-4 py-2 text-sm font-semibold text-white">Clock out</button>
            )}
            <button disabled={busy === j.id} onClick={() => { const note = prompt("What's the issue?"); if (note) fire("/api/crm/clock", { job_id: j.id, event: "exception", note }, j.id); }}
              className="rounded-lg bg-stone-200 px-4 py-2 text-sm dark:bg-stone-700">Report issue</button>
            <label className="cursor-pointer rounded-lg bg-stone-200 px-4 py-2 text-sm dark:bg-stone-700">
      ...
      {!jobs.length && <p className="text-stone-400">No stops today. 🌤</p>}
    </div>
  );
}
